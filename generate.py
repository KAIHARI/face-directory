...
(function() {
  ...
  window.generatePhotobook = function() {
    ...
    const PAD_OUTER = 36 * DPI;       // outer canvas padding
    const GUTTER = 32 * DPI;          // space between columns
    const COL_W = (CANVAS_W - PAD_OUTER * 2 - GUTTER) / COLS;
    const PHOTO_SIZE = 260 * DPI;     // bumped 220 → 260 (more recognizable)
    const GAP = 22 * DPI;              // space between photo and text
    const ROW_PAD = 22 * DPI;         // vertical padding inside cell
    const CELL_H = PHOTO_SIZE + ROW_PAD * 2;
    const HEADER_H = 140 * DPI;
    const ROWS = Math.ceil(TOTAL / COLS);
    const CANVAS_H = HEADER_H + ROWS * CELL_H + PAD_OUTER;
    ...
    ctx.font = '700 ' + (52 * DPI) + 'px Georgia, serif';
    ctx.textAlign = 'center';
    ctx.fillText('Staff Directory', CANVAS_W / 2, 78 * DPI);

    // Subtitle line
    ctx.fillStyle = '#999';
    ctx.font = '400 ' + (18 * DPI) + 'px sans-serif';
    const date = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
    ctx.fillText(date, CANVAS_W / 2, 108 * DPI);

    // Header divider
    ctx.strokeStyle = '#DDD';
    ctx.lineWidth = 1 * DPI;
    ctx.beginPath();
    ctx.moveTo(PAD_OUTER, HEADER_H - 6 * DPI);
    ctx.lineTo(CANVAS_W - PAD_OUTER, HEADER_H - 6 * DPI);
    ctx.stroke();
    ...
    for (let i = 0; i < TOTAL; i++) {
      ...
      const x = PAD_OUTER + col * (COL_W + GUTTER);
      const y = HEADER_H + row * CELL_H + ROW_PAD;
      ...
      // Subtle border around photo for crispness on print
      ctx.strokeStyle = '#E8E4DC';
      ctx.lineWidth = 1 * DPI;
      ctx.beginPath();
      ctx.roundRect
        ? ctx.roundRect(photoX, photoY, PHOTO_SIZE, PHOTO_SIZE, r)
        : ctx.rect(photoX, photoY, PHOTO_SIZE, PHOTO_SIZE);
      ctx.stroke();

      // Text area to the right of the photo
      const textX = x + PHOTO_SIZE + GAP;
      const textMaxW = COL_W - PHOTO_SIZE - GAP;
      const displayNum = String(i + 1).padStart(2, '0');
      ...
      let posSize = 23 * DPI;
      let posLH = 29 * DPI;
      const NUM_SIZE = 15 * DPI;

      // Auto-shrink name if it would wrap to 3+ lines
      ctx.font = '700 ' + nameSize + 'px sans-serif';
      const name = (saved[i] && saved[i].name && saved[i].name.trim()) || '(not filled)';
      let nameLinesArr = computeWrappedLines(ctx, name, textMaxW);
      if (nameLinesArr.length >= 3) {
        nameSize = 30 * DPI;
        nameLH = 36 * DPI;
        ctx.font = '700 ' + nameSize + 'px sans-serif';
        nameLinesArr = computeWrappedLines(ctx, name, textMaxW);
      }

      // Auto-shrink position if it would wrap to 3+ lines
      ctx.font = '400 ' + posSize + 'px sans-serif';
      const pos = (saved[i] && saved[i].position && saved[i].position.trim()) || '(not filled)';
      let posLinesArr = computeWrappedLines(ctx, pos, textMaxW);
      if (posLinesArr.length >= 3) {
        posSize = 19 * DPI;
        posLH = 24 * DPI;
        ctx.font = '400 ' + posSize + 'px sans-serif';
        posLinesArr = computeWrappedLines(ctx, pos, textMaxW);
      }

      const numBlockH = NUM_SIZE + 10 * DPI;
      const nameBlockH = nameLinesArr.length * nameLH;
      const posBlockH = posLinesArr.length * posLH;
      const gapBetween = 10 * DPI;
      const totalH = numBlockH + nameBlockH + gapBetween + posBlockH;

      // Vertically center the text block against the photo
      let textY = y + (PHOTO_SIZE - totalH) / 2;
      if (textY < y + 4 * DPI) textY = y + 4 * DPI; // never overflow top

      // Safety clip region so nothing can visually overflow the text column
      ctx.save();
      ctx.beginPath();
      ctx.rect(textX, y, textMaxW, PHOTO_SIZE);
      ctx.clip();

      // Card number (small muted label)
      ctx.fillStyle = '#B0B0B0';
      ctx.font = '600 ' + NUM_SIZE + 'px sans-serif';
      ctx.fillText('#' + displayNum, textX, textY + NUM_SIZE);
      textY += numBlockH;

      // Name (big, bold, dark) — draw pre-computed lines
      ctx.fillStyle = '#111';
      ctx.font = '700 ' + nameSize + 'px sans-serif';
      drawLines(ctx, nameLinesArr, textX, textY + nameSize * 0.85, nameLH);
      textY += nameBlockH + gapBetween;

      // Position (medium, regular, secondary color)
      ctx.fillStyle = '#555';
      ctx.font = '400 ' + posSize + 'px sans-serif';
      drawLines(ctx, posLinesArr, textX, textY + posSize * 0.85, posLH);

      ctx.restore();

      // Full-row divider after the right column
      if (col === COLS - 1 && i < TOTAL - 1) {
        ctx.strokeStyle = '#EEE';
        ctx.lineWidth = 1 * DPI;
        ctx.beginPath();
        ctx.moveTo(PAD_OUTER, y + PHOTO_SIZE + ROW_PAD);
        ctx.lineTo(CANVAS_W - PAD_OUTER, y + PHOTO_SIZE + ROW_PAD);
        ctx.stroke();
      }
    }
  ...
  // Split a string into atoms that can start a new line.
  // Breaks on spaces AND after hyphens (keeps the hyphen attached to the previous atom).
  function splitForWrap(text) {
    // First split by spaces
    const words = text.split(' ');
    const atoms = [];
    for (const word of words) {
      // Then split each word on hyphens, keeping the hyphen at the end
      if (word.includes('-')) {
        const parts = word.split('-');
        for (let i = 0; i < parts.length; i++) {
          const isLast = i === parts.length - 1;
          if (parts[i] !== '') atoms.push(parts[i] + (isLast ? '' : '-'));
        }
      } else {
        atoms.push(word);
      }
  ...
  // Build the wrapped lines (array of strings) without drawing.
  // Handles space-split, hyphen-split, and character-level fallback for
  // atoms that are individually wider than maxWidth.
  function computeWrappedLines(ctx, text, maxWidth) {
    const atoms = splitForWrap(text);
    const lines = [];
    let line = '';
    ...
    const charFallback = (atom) => {
      // Break an overlong atom character by character
      let current = '';
    ...
    for (const atom of atoms) {
      // If the atom itself is wider than maxWidth, flush current line and char-break it
      if (ctx.measureText(atom).width > maxWidth) {
        push();
        charFallback(atom);
        continue;
      }
      // Normal word-fit check. Only add a space if the current line doesn't
      // already end in a hyphen (hyphens glue to the previous atom).
      const sep = line && !line.endsWith('-') ? ' ' : '';
      const testLine = line + sep + atom;
  ...
  // Draw pre-computed lines at the given baseline
  ...
  // Backwards-compatible wrappers (previously used signatures)
  ...
  function getWrappedLines(ctx, text, maxWidth) {
    return computeWrappedLines(ctx, text, maxWidth).length;
  }
